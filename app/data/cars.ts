// Shared cars dataset (extracted from the page payload)
const CDN_BASE_URL = process.env.NEXT_PUBLIC_CDN_BASE_URL || "http://vehicle-photos-published.vauto.com";

export const cars: any[] = [
    {
        id: "0280a388-6aa6-4ea1-b50d-d93ce1ddd06e",
        vin: "KNAG64J71T5425442",
        dealer_id: "MP2227",
        stock_id: "N34958",
        condition: "N",
        year: 2026,
        make: "Kia",
        model: "K5",
        model_code: "L4252",
        body: "4D Sedan",
        transmission: "8-Speed Automatic",
        series: "GT-Line",
        door_count: 4,
        odometer: "6",
        engine_cylinder: 4,
        engine_displacement: 2.5,
        drive_train_desc: "FWD",
        exterior_color: "Yacht Blue",
        interior_color: "Black",
        generic_exterior_color: "Blue",
        price: "$0.00",
        msrp: "$29,585.00",
        inventory_date: "2025-09-03",
        certified: "No",
        description: "Yacht Blue 2026 Kia K5 GT-Line FWD I4 4D Sedan 8-Speed Automatic...",
        features: [
            "Wheels: 18\" x 7.5J Gloss Black Machined Finish Alloy",
            "Heated Front Bucket Seats"
        ],
        city_mpg: 25,
        highway_mpg: 36,
        photos: [
            `${CDN_BASE_URL}/7d/76/bc/35-4b5e-4951-b551-60fb7c377e26/image-1.jpg`
        ],
        engine: "I4",
        fuel: "Gasoline",
        age: 50,
        available_for_sale: true,
        special_offer: true,
        store: {
            name: "Kia of Bedford",
            gps_longitude: -81.6177,
            gps_latitude: 41.3863,
            address: {
                street_number: "18180",
                street_name: "Rockside Rd",
                city: "Bedford",
                state: "OH",
                zip_code: "44146"
            }
        }
    },
    {
        id: "0094a32d-da2e-455b-b70b-41bc30face7e",
        vin: "KNDETCA70T7883586",
        dealer_id: "MP2227",
        stock_id: "N35274",
        condition: "N",
        year: 2026,
        make: "Kia",
        model: "Seltos",
        model_code: "K4482",
        body: "4D Sport Utility",
        transmission: "8-Speed Automatic",
        series: "SX",
        door_count: 4,
        odometer: "4",
        engine_cylinder: 4,
        engine_displacement: 1.6,
        drive_train_desc: "AWD",
        exterior_color: "Snow White Pearl",
        interior_color: "Black",
        price: "$0.00",
        msrp: "$31,390.00",
        inventory_date: "2025-10-01",
        certified: "No",
        description: "Snow White Pearl 2026 Kia Seltos SX AWD I4 4D Sport Utility...",
        features: ["Wheels: 18\" Alloy w/Machined Finish"],
        city_mpg: 24,
        highway_mpg: 27,
        photos: [
            `${CDN_BASE_URL}/27/89/53/67-9410-4c48-9648-00d1a819d8db/image-1.jpg`
        ],
        engine: "I4",
        fuel: "Gasoline",
        age: 22,
        available_for_sale: true,
        special_offer: true,
        store: { name: "Kia of Bedford" }
    },
    {
        id: "06c0e09a-4248-4a9d-8e64-55ae668f8589",
        vin: "KNAG64J78T5401882",
        year: 2026,
        make: "Kia",
        model: "K5",
        msrp: "$29,585.00",
        photos: [
            `${CDN_BASE_URL}/da/90/7d/42-5d3e-4366-8fba-fb9115025d63/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "0400f27f-a2dd-470c-bb9c-7344a1286eac",
        vin: "5XYK23DFXTG364508",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$30,335.00",
        photos: [
            `${CDN_BASE_URL}/d5/e7/c2/47-5afe-4c0d-a893-c16382419ffa/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "026c33bc-fc62-440b-a825-9e89b5afdc4d",
        vin: "5XYK23DF5TG365775",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$31,065.00",
        photos: [
            `${CDN_BASE_URL}/f3/d3/db/cd-6a7d-4d16-b467-ebdcb8ad7b44/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "0130e608-dbc7-4b1d-88e3-e69428a401d3",
        vin: "5XYK33DF6TG382212",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$30,490.00",
        photos: [
            `${CDN_BASE_URL}/31/3d/12/1c-78b3-4474-a9fe-21e996749659/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "077b24e9-00d5-4e8f-ae57-72802ba56e06",
        vin: "KNDRMDLH3N5070431",
        year: 2022,
        make: "Kia",
        model: "Sorento Plug-In Hybrid",
        price: "$29,997.00",
        msrp: "$48,090.00",
        photos: [
            `${CDN_BASE_URL}/58/81/56/f4-6dba-4be5-b32b-1e7ad11e7c2d/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "05bb65c4-f59c-4f12-9cd0-3d65784e1a99",
        vin: "5XYK6CDF9TG368456",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$35,130.00",
        photos: [
            `${CDN_BASE_URL}/e3/c1/d6/41-f2c6-42d2-8866-8d350b337714/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "016f67f6-2fa7-4e17-a8d3-49a60a03cf34",
        vin: "5XYP6DGC1RG487530",
        year: 2024,
        make: "Kia",
        model: "Telluride",
        price: "$34,997.00",
        msrp: "$40,315.00",
        photos: [
            `${CDN_BASE_URL}/b5/40/06/1a-2c7b-4361-8462-34a2872962f9/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "03c44202-5f53-4a69-a8b5-124583e65c4d",
        vin: "5XYK6CDF1TG355748",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$34,565.00",
        photos: [
            `${CDN_BASE_URL}/de/c9/21/a6-07b8-4b7b-bb6c-73ea41d9341f/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "03ae8024-2234-48e3-b59c-b1668bacf4c2",
        vin: "5XYK23DF9TG364774",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$30,265.00",
        photos: [
            `${CDN_BASE_URL}/bf/c5/b1/36-733b-4c91-9834-36fc8bd34f08/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "057b70ec-83f6-455d-b2d7-e36dee691d9f",
        vin: "KNDEUCAA5T7834490",
        year: 2026,
        make: "Kia",
        model: "Seltos",
        msrp: "$28,870.00",
        photos: [
            `${CDN_BASE_URL}/97/d7/82/98-d7ea-4d42-b3a6-2c881983604d/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "06ae022d-2b67-4703-96a0-9918f21d44c3",
        vin: "KNAG64J75T5413777",
        year: 2026,
        make: "Kia",
        model: "K5",
        msrp: "$31,985.00",
        photos: [
            `${CDN_BASE_URL}/74/e5/84/65-1691-4b13-99da-5845b22c4d6d/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "04bfc862-a0d5-4f4a-81c2-7bb1a3044c28",
        vin: "5XYK53DF4TG372241",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$36,290.00",
        photos: [
            `${CDN_BASE_URL}/64/6c/6d/bf-acee-415b-a0c4-817d66aa4a97/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "049959d4-7f9e-4f8c-84ed-cac3f62b7525",
        vin: "5XYK23DF8TG367102",
        year: 2026,
        make: "Kia",
        model: "Sportage",
        msrp: "$28,690.00",
        photos: [
            `${CDN_BASE_URL}/08/71/9a/dc-0414-4612-b568-e3ed1bac1f3d/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    },
    {
        id: "01118f47-875c-4c7f-8e74-78a527c8b682",
        vin: "5XYRG4JC6SG397102",
        year: 2025,
        make: "Kia",
        model: "Sorento",
        msrp: "$33,630.00",
        photos: [
            `${CDN_BASE_URL}/71/3d/9f/73-3089-45ec-a142-1e46efef27d7/image-1.jpg`
        ],
        store: { name: "Kia of Bedford" }
    }
];

export default cars;
